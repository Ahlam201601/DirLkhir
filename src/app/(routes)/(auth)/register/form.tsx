...
import { registerSchema, type RegisterValues } from "./validate";
import InputPasswordContainer from "../components/input-password";
import InputStartIcon from "../components/input-start-icon";
import { cn } from "@/lib/utils";
import { Mail, User, Lock, Loader2, UserPlus } from "lucide-react";
...
export default function RegisterForm() {
  ...
  const getInputClassName = (field: keyof RegisterValues) =>
    cn(
    ...
    <Form {...form}>
      ...
      >
          ...
          render={({ field }) => (
            <FormItem className="space-y-3">
              ...
              <FormControl>
                <InputPasswordContainer>
                  <Input
                    className={cn("pe-9 text-base", getInputClassName("confirmPassword"))}
                    placeholder="••••••••"
                    disabled={isPending}
                    {...field}
                  />
        ...
        <p className="text-muted-foreground text-center text-sm">
          Déjà un compte ?{" "}
          <Link href="/login" className="text-primary hover:underline underline-offset-4 transition-all duration-200 font-medium">
            Se connecter
          </Link>
        </p>
      </form>
